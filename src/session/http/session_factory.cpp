...
namespace myhttpd::http {
    ...
    std::unique_ptr<myhttpd::session> session_factory::create_session(std::unique_ptr<myhttpd::network::connection> conn) {
        return std::make_unique<myhttpd::http::session>(std::move(conn), this->_resource, this->_ctx);
    }
