...
namespace myhttpd {
    void acceptor::_handshake_handler(std::unique_ptr<tls_soc> soc, const boost::system::error_code &error) {
        if(!error) {
            this->_nca_handler(std::make_unique<connection::secure_connection>(std::move(*soc)));
        } else {
        }
    }
    ...
    void acceptor::_accept_handler(const boost::system::error_code &error, ordinary_soc peer) {
        if(!error) {
            this->_nca_handler(std::make_unique<connection::insecure_connection>(std::move(peer)));
    ...
    : _io(io) {
        ...
        // if(config->Attribute("use_https") && config->Attribute("use_https") == "ture") {
        //     if()
        // }
