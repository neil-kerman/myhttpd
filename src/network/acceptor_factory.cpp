...
namespace myhttpd::network {
    ...
    std::unique_ptr<acceptor> acceptor_facory::create_acceptor(tinyxml2::XMLElement* config, boost::asio::io_context &ctx) {
        std::string name = config->Name();
        std::string address = config->Attribute("address");
        int port = config->IntAttribute("port");
        std::unique_ptr<acceptor> ac;
        if (name == "tcp") {
            ac.reset(new tcp_acceptor(address, port, ctx));
            return ac;
        } else if (name == "tls") {
            ...
            ac.reset(new tls_acceptor(address, port, ctx, std::move(*tls_ctx)));
            return ac;
        } else {
            return nullptr;
        }
